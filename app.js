...
const API_URL = 'https://api.sportradar.us/soccer-t3/eu/en/schedules/2017-04-07/schedule.json?api_key=n387hsw9p7r8v59pkc85vkmy';
	...
	axios.get(url)
	  .then(function (results) {
			...
			const games = data.map(function(x){
				...
				return `<div><li>${x.scheduled}</li><li>${x.season.name}</li>` + teams + '</div>';
			});
	    console.log(games);
